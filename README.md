...
- 声は「設定 → 通話の声」で変えられます。保存後、通話を開始し直すと切り替わります（アプリの再起動は不要）。
  候補：Leda（若く明るい）/ Zephyr（明るい）/ Aoede（軽やか）/ Puck（元気）/ Kore（落ち着き）/ Charon（低め）/ Fenrir（力強い）/ Orus（しっかり）。
  聞き比べは Google AI Studio の Live（Stream Realtime）で行えます。無効な名前を指定すると、通話開始時にその名前を添えてエラーが出ます
- 読み上げは「設定 → 通話の読み上げ」で選べます。
  - **Gemini**（既定）：追加の用意は不要。声は「通話の声」で選びます
  - **PCの読み上げエンジン**：PCで [AivisSpeech](https://aivis-project.com/) か [VOICEVOX](https://voicevox.hiroshiba.jp/) を起動しておくと、その声で読み上げます。
    URLは AivisSpeech が `http://127.0.0.1:10101`、VOICEVOX が `http://127.0.0.1:50021`。話者とスタイルはエンジン上の表示名をそのまま入れます（例：コハク／ノーマル）。
    キャラクターごとの利用規約・クレジット表記の条件は、各エンジンの配布元で確認してください
  - PCの読み上げエンジンを選んでも繋がらない場合は、Geminiの声で通話を続け、理由を画面に出します
  - PCの読み上げを使うと、文がそろってから合成するぶん、最初の一声までがGeminiより少し遅くなります
  - PCの読み上げ中も、Gemini側は音声を生成します（そのモデルは音声出力専用で、文字だけを返せないため）。
    生成された音声は再生せず、その書き起こしを読み上げに使います。Gemini側の利用量は音声のときと同じです
- 「設定 → 話し方」で、トーン・語尾・テンポを言葉で指示できます。**声の印象は名前よりこちらで大きく変わります**（例：「少し高めの明るいトーンで、やわらかい語尾で」）。空にすると指示なしになります
