...
自動整理をONにしていても、日本時間03:00台だけ実行します。
**起動直後の未処理回収は行いません。** 日中に起動しただけでGemini APIをバックグラウンド実行しないためです。
