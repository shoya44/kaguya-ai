...
- 「これ覚えておいて」による即時記憶
- 指定時刻のリマインダー
- リマインダーは「確認しました」を押すまで保持（複数件は順に表示、次の通知は最大約30秒後）
- リマインダーの時刻にはPC版の画面を前に出し、前に出せないときはタスクバーを点滅させます。全画面共通の通知から「かぐやと話す」で音声通話を始められます。確認済みにする操作は「確認しました」です
- ローカルカレンダー
- Open-Meteoによる天気取得
...
- 自動整理の1日上限（設定値。1〜10回、既定3）を超えたら、その日の**自動実行は**打ち切り。「未処理の会話を記憶へ反映」は上限に関係なく実行できます
- **失敗したときの歯止め**：API呼び出しが失敗したら使った枠を戻します。ただし同じ日に2回続けて失敗したら、その日の自動整理は止めます（直らない理由で叩き続けないため）。「未処理の会話を記憶へ反映」は止めません
...
「設定 → 未処理の会話を記憶へ反映」は手動でいつでも実行できます。1回押すと最大5バッチ（約300件）まで処理して止まります。結果には会話の処理件数、記憶更新の延べ件数、残件数を表示します。既存記憶全体の重複整理は行いません。足りなければもう一度押してください。1日の上限はありません。整理失敗時は、分かる範囲で利用制限・タイムアウト・結果形式不正・保存失敗を区別して表示します。
...
| `auto_call_limit` | 自動整理の1日上限。**自動整理と週次更新だけ**が使います。「未処理の会話を記憶へ反映」は上限で止まりません | 1〜10回、既定3 |
...
- 1回最大10分。残り時間と終了前の予告を表示し、終了後は「通話を再開」で接続し直せます。画面を離れた場合も終了理由を表示します
- マイク音声は16kHz PCMとしてGemini Liveへ送信し、返答音声をその場で再生（1回最大10分・従量課金）
- 通話中は通話ボタンの左に音量のつまみと消音ボタンが出ます。読み上げがGeminiでもPCのエンジンでも同じつまみで効きます
- 音量・消音はかぐやの声だけに適用します。「マイクを切る／入れる」で自分の声の送信を別に切り替えられます。マイクは新しい通話ではオンになります。音量と消音は**端末ごと**に覚えます。PCとiPhoneで別々に設定できます（サーバーには送りません）
...
- 音声通話からも、指定時刻のリマインダー予約と「覚えておいて」は実行します
- 音声通話から天気・予定・ファイル検索・BAT等の外部操作は実行しません
...
時刻になると、設定・記憶画面でも共通の通知に期限と内容を表示します。PC版では画面が前に出ます。
別のウィンドウを操作中で前に出せないときは、タスクバーのアイコンが点滅します。
本文のタップでは確認済みにしません。「確認しました」で保存し、「かぐやと話す」は確認とは別に音声通話を始めます。通知は接続中のアプリ内表示で、iPhoneのロック画面には届きません。
通話できない接続（同一LANのHTTPなど）では、このボタンは出ません。
...
「設定 → 未処理の会話を記憶へ反映」で未処理の会話から長期的な情報を抽出します。
...
- Web Push
- 音声通話からの天気・予定・PC操作などの外部ツール実行（リマインダー予約と記憶は実行します）
- Windowsのトースト通知（`tauri-plugin-notification` が要りますが、通常の起動は依存を取りに行かない方針のため見送り。代わりに前面化とタスクバーの点滅で知らせます）
- MP4以外の動画形式のファイルタブ再生
- 配布用インストーラー
- LLMによる自己ソース書き換え
