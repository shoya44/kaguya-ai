...
THINK_DELAY_MAX = 1.2
# 間の揺れ幅（比率）。0.9秒きっかりが毎回続くと、間そのものが規則として見える。
# 待つと決めた回だけ、この割合の範囲で伸び縮みさせる。
THINK_DELAY_JITTER = .35
# 長文は読む時間ぶん遅れる。この字数を超えた分に対して、100字あたりこの秒数。
# 短い雑談は従来どおり即答（0秒）のまま。
THINK_DELAY_READ_FROM = 80
THINK_DELAY_READ_PER_100 = .35


# --- 返答の分量の揺れ ------------------------------------------------------
# 同じ気分なら毎回同じ分量で返ってくる、という規則性を崩す。短めへ振る確率と
# 長めへ振る確率がそれぞれこの値で、残りは従来どおり分量の指示なし。
REPLY_LENGTH_SWAY = .15
