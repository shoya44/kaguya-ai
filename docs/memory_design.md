...
いまの感情から符号を1つ求め、`tone` と一致する記憶を優先する。
...
|---|---|---|
| 0 | この設計文書 | — |
| 1 | マイグレーションとリネーム（**振る舞いは変えない**）、`living_activity` の新設とフロントからの移行 | 0 |
| 2 | ① 記憶 → 感情（想起結果の再利用、処理順の入れ替え） | 1 |
| 3 | ③ 想起スコア（`tone` の付与と気分の一致） | 1, 2 |
